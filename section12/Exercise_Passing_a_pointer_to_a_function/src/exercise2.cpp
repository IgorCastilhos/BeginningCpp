...
int find_max_element(const int *arr, int size);

int find_max_element(const int *arr, int size) {
    int maxElement = *arr;  // maxElement é o primeiro elemento
    for (int i{0}; i < size; ++i) {
        if (*(arr + i) > maxElement)
            maxElement = *(arr + i);  // atualiza maxElement se um elemento maior for encontrado
    }
    return maxElement;
}
